...
// Ником. Без оплаты и без базы данных (низкий ожидаемый объём на старте —
// та же логика, что у /sovet/ на nikolablajen.ru): просто уведомление по
// email, Ник отвечает сам, лично, кому откликнется.
const NOTIFY_EMAIL = 'nchechelnitskiy@gmail.com'
// Основной домен kodeksdeneg.ru отдаётся статикой через GitHub Pages
// (Cloudflare) — эта функция реально доступна только через поддомен
// bot.kodeksdeneg.ru (остаётся на Vercel, см. api/tg.js). Страница
// /sovetnik/ шлёт запрос именно туда — значит это кросс-доменный
// запрос, нужен CORS.
const ALLOWED_ORIGIN = 'https://kodeksdeneg.ru'
  ...
  if (apiKey) {
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Кодекс денег <app@nikolablajen.ru>',
          to: NOTIFY_EMAIL,
          subject: 'Заявка на «Советник» — Кодекс денег',
          html: `
            <div style="font-family: -apple-system, sans-serif; padding: 24px;">
              <h2>Новая заявка на «Советник» (kodeksdeneg.ru)</h2>
