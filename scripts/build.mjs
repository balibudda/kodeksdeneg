...
<script type="text/javascript">
  window.kdLoadMetrika = function () {
    ...
    ym(112520018, 'init', {ssr:true, webvisor:true, clickmap:true, ecommerce:"dataLayer", referrer: document.referrer, url: location.href, accurateTrackBounce:true, trackLinks:true});
  };
...
<p>Сайт использует cookies и сервис веб-аналитики Яндекс.Метрика, чтобы понимать, как посетители пользуются справочником. Аналитика подключается только после вашего согласия (кнопка в нижней плашке). Что именно собирается и зачем — в <a href="/politika/">Политике обработки персональных данных</a>.</p>`
  return layout({
...
<p>Обработка ведётся на основании согласия посетителя, выражаемого путём нажатия кнопки «Принять» в информационной плашке о cookie, а также на основании законных интересов Оператора по обеспечению работы Сайта. До получения согласия сервис веб-аналитики не загружается.</p>

<h2>5. Использование cookie и веб-аналитики</h2>
<p>Сайт использует сервис <b>Яндекс.Метрика</b>, предоставляемый ООО «ЯНДЕКС». Сервис использует cookie и технологию Вебвизор (запись действий на странице в обезличенном виде). Условия обработки данных Яндекс.Метрикой — на сайте Яндекса (<a href="https://yandex.ru/legal/metrica_agreement/" target="_blank" rel="noopener noreferrer">yandex.ru/legal/metrica_agreement</a>). Отключить сбор можно, нажав «Отклонить» в плашке, либо запретив cookie в браузере, либо установив блокировщик Яндекс.Метрики.</p>
...
<p>Обезличенные данные веб-аналитики хранятся в течение срока, установленного сервисом Яндекс.Метрика. Значение согласия на cookie хранится в браузере посетителя (localStorage) до его удаления посетителем.</p>
...
<p>Оператор вправе изменять настоящую Политику. Актуальная редакция всегда доступна по адресу ${SITE.origin.replace('https://', '')}/politika/. Дата последнего обновления указана в начале страницы.</p>`
  return layout({
    ...
    description: 'Как сайт «Кодекс денег» обрабатывает данные: cookie и Яндекс.Метрика только после согласия, никакой идентификации личности.',
