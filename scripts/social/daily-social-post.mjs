...
// ролик по каждой и публикует на подключённые каналы — YouTube +
// Facebook + TikTok. По умолчанию 3 темы за прогон (Ник, 13.09.2026:
// «каждый день три новости» — другой темп, чем у kodeksdetstva, там
// 1/день).
//
// 16.09.2026, по просьбе Ника: из этих 3 тем — 2 теперь в комедийном
// формате Рублёнка (тот же движок, что и вечером), третья — как раньше,
...
//
// Обновляет posted-social.json — коммитит и пушит уже сам workflow, не
// этот скрипт.
//
...
// 16.09.2026, по просьбе Ника: 2 из 3 утренних тем теперь делаются в
// комедийном формате Рублёнка (тот же движок, что и вечером), третья —
// как раньше, обычным форматом. Одноразовый файл — пишет write-script
// job перед этим скриптом, здесь только читаем и используем.
