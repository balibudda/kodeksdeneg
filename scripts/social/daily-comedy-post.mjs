// Вечерний автопилот НОВОГО комедийного формата (14.09.2026). Отдельная
// очередь и трекер от утреннего daily-social-post.mjs — форматы не
// смешиваются, тема может быть опубликована в обоих независимо.
//
// Логика:
...
//     затем удаляем файл (он одноразовый, в git не попадает).
//  2. Иначе — берём PILOT из generate-comedy-short.mjs (первый прогон,
//     когда очередь ещё пуста — тот самый ролик, одобренный Ником).
//
