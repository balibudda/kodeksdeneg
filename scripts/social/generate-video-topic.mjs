...
// водяной знак kodeksdeneg.ru. Формат — тот же, что подтверждён Ником
// 13.09.2026 на kodeksdetstva: реальный скрин страницы, без дублирования
// заголовка текстом, непрозрачная плашка субтитров, длинный текст поста.
//
...
export const SITE = { name: 'Кодекс денег', origin: 'https://kodeksdeneg.ru' }
...
// текст вручную, заполнять реальный объём площадки). Используем весь
// контент темы. У kodeksdeneg нет полей say/ages (в отличие от kodeksdetstva).
export function buildCaption(topic) {
  ...
  return [
    ...
    '',
    `🧭 ${SITE.name} — бесплатный справочник о деньгах, долгах и правах должника.`,
  ].join('\n')
}
...
export async function renderTopicVideo(slug, outDir) {
  ...
  const narrationText = `${topic.title}. ${sutText} ${firstStep} Полный разбор — на сайте кодекс денег точка ру.`
  ...
  filterParts.push(`drawtext=fontfile=${regularFont}:text='kodeksdeneg.ru':fontcolor=0x8A806E:fontsize=30:x=(w-text_w)/2:y=h-60:box=0`)
...
async function main() {
  ...
  if (!slug || !TOPICS_BY_SLUG[slug]) throw new Error(`тема не найдена: ${slug}. Например: ${TOPICS.slice(0, 5).map((t) => t.slug).join(', ')}...`)
